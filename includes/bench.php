<section class="video-container">
<div class="titre-infos">
  <h3>Benchmark</h3>
</div>
<div class="video">
    <?php echo $config['bench-1-link'] ?>
    <?php echo $config['bench-2-link'] ?>
    <?php echo $config['bench-3-link'] ?>
</div>

<div class="img-gpu">
  <img src="../medias/gpucheck.png" alt=""><p>
    <a target="_blank" href="https://www.tomshardware.com/reviews/gpu-hierarchy,4388.html">GPU Benchmarks and Hierarchy 2023: Graphics Cards Ranked</a></p>
    <a target="_blank" href="https://cdn.mos.cms.futurecdn.net/q56doGjaC7kHDZkQvjJpZh.png">Ouvrir en grand</a></p> 
</div>
</section>